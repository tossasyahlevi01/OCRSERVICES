...
namespace SERVICEOCR2.Controllers
{
    public class OCRServiceController : Controller
    {
        // GET: OCRService
        ...
        [HttpPost]
        public JsonResult gethasil(files f)
        {
          //  var bitmap = new Bitmap(Server.MapPath(f.path));
            string folderpath = Server.MapPath("~/IMAGES/");

            //  upload.SaveAs(folderpath + Path.GetFileName(upload.FileName));
            string filepath = folderpath + Path.GetFileName(f.path);
            //imgori.ImageUrl = "~/IMAGESCONVERT/" + Path.GetFileName(upload.FileName);

            string ext = System.IO.Path.GetExtension(filepath);
            if (ext.ToLower() == ".jpg" || ext.ToLower() == ".jpeg")
            {
                //string Hasil = this.process2(bitmap);
              //  getresult gt = new getresult { hasil = Hasil };
                getresult gt = new getresult { hasil = "ok"};
            ...
            {
                getresult gt = new getresult { hasil = Path.GetFileName(filepath)};
                return Json(gt, JsonRequestBehavior.AllowGet);
